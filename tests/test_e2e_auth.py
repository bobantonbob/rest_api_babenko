...
#     # Мокуємо функцію send_email
#     mock_send_email = Mock()
#     monkeypatch.setattr("src.routes.auth.send_email", mock_send_email)
#     # Викликаємо API для створення нового облікового запису
#     response = client.post("api/auth/signup", json=user_data)
#     # Перевіряємо, чи повертається код статусу 409 (конфлікт)
#     assert response.status_code == 409, response.text
#     # Перевіряємо, що повертається повідомлення про наявність облікового запису
#     data = response.json()
#     assert data["detail"] == messages.ACCOUNT_EXIST
#     # Перевіряємо, чи була викликана функція send_email з правильними аргументами
